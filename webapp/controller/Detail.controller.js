...
sap.ui.define([
	...
	"ZZ1/ZZSOCIAL_MEDIA_EX/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function (BaseController, JSONModel, formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("ZZ1.ZZSOCIAL_MEDIA_EX.controller.Detail", {
		...
		onDelete: function () {
			...
			var oViewModel = this.getModel("detailView"),
				...
				sObjectHeader = this._oODataModel.getProperty(sPath + "/PartnerID"),
		...
		_onObjectMatched: function (oEvent) {
			...
			this.getModel().metadataLoaded().then(function () {
				var sObjectPath = this.getModel().createKey("ZZ1_BPSOCIAL_ACC", oParameter);
		...
		_onBindingChange: function () {
			...
			var sPath = oElementBinding.getBoundContext().getPath(),
				...
				sObjectId = oObject.SAP_UUID,
				sObjectName = oObject.PartnerID;
		...
		_callDelete: function (aPaths, fnAfterDeleted) {
			...
			var fnFailed = function () {
				this._oODataModel.setUseBatch(this.setUseBatch(true));
			}.bind(this);
		...
		onAddAccount: function (oEvent) {
			...
			this.getModel("appView").setProperty("/addEnabled", false);
			var sObjectPath = this.getView().getElementBinding().getPath();
			this.getRouter().getTargets().display("createMedia", {
				mode: "create",
				objectPath: sObjectPath
			});
			/*		var partnerUID = this._oODataModel.getProperty("UUID");
				var oContext = this._oODataModel.createEntry("ZZ1_MEDIAACCOUNT_BPSOCIAL_ACC",{ properties:{SAP_PARENT_UUID:partnerUID},
		...
		_fnEntityCreated: function (oData) {
			/*	var sObjectPath = this.getModel().createKey("ZZ1_BPSOCIAL_ACC", oData);
		...
		onDeleteLine: function (oEvent) {
			var sObjectPath = oEvent.getSource().getBindingContext().getPath();
			var oViewModel = this.getModel("detailView"),
				sObjectHeader = this._oODataModel.getProperty(sObjectPath + "/Service"),
				sQuestion = this._oResourceBundle.getText("deleteText", sObjectHeader),
				sSuccessMessage = this._oResourceBundle.getText("deleteSuccess", sObjectHeader);
